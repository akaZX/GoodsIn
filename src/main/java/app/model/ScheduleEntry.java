...
public class ScheduleEntry extends RecursiveTreeObject<ScheduleEntry> {
    ...
    public Entry<?> generateCalendarEntry() {
        ...
        String pallets = this.getDetails().getPallets() + (this.getDetails().getPallets() > 1 ? " pallets"
                : " pallet");
        String comments =
                this.getDetails().getComments() == null ? "" : (" | " + this.getDetails().getComments());
        String trailerRegistration =
                this.getDetails().getRegistrationNo() == null ? "" : (" | " + this.getDetails().getRegistrationNo());
        String entryTitle =
                this.getSupplier().getSupplierName() + " | " + this.getOrder().getPoNumber() + " | " +
                this.getDetails().getHaulier();
